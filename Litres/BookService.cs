...
public class BookService
{
    ...
    public List<dynamic> GetAuthorOfBookByHisId(int id)
    {
        using var connect = new NpgsqlConnection(_connectionString);
        return connect.Query("select a.FullName as AuthorName from BookAuthors as ba join Books as b on ba.BookId = b.Id join Authors as a on ba.AuthorId = a.Id where b.Id = @id",new{Id = id}).ToList()!;
    }
}
